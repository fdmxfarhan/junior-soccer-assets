...
bool just_seen_ball = false;
bool is_ball_front = false;
int ball_seen_time = 0;
...
void forward() {
  if (!digitalRead(PA3) && HS0038.is_ball && (HS0038.angle < 30 || HS0038.angle > 330)) {
    ...
    if (a < -180) a += 360;
    if(just_seen_ball){
      just_seen_ball = false;
      ball_seen_time = millis();
      if(abs(a) < 60) is_ball_front = true;
      else is_ball_front = false;
    }
    if(is_ball_front && millis() - ball_seen_time < 4000){
      float shift = clamp(a * 0.30, -60, 60);
      moveAngle(a + shift, 55000);
      return;
    }
    ...
    moveAngle(a + clamp(output, -60, 60), forward_v);
