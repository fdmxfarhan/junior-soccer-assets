...
void motor(int ML1, int ML2, int MR2, int MR1) {
  float correction = clamp(angle_robot, -90, 90);
...
void forward() {
  ...
  if (!digitalRead(PA3)) {
    ...
    float inertia = 0;
    if(ball_dir == 's' && a > 50) ball_dir = 'r';
    if(ball_dir == 's' && a < -50 ) ball_dir = 'l';
    if(a > -40 && a < 40){
      if(ball_dir == 'r') inertia = -20;
      if(ball_dir == 'l') inertia = 20;
    }
    moveAngle(a + shift + inertia, forward_v);
  } else {
