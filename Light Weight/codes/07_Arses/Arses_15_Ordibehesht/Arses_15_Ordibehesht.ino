...
float shift_integrator = 0;
int shift_integrator_intervar_time = 0;
void forward() {
  out();
  if (!digitalRead(PA3) && HS0038.is_ball && (HS0038.angle < 30 || HS0038.angle > 330)) {
    ...
    if(millis() - shift_integrator_intervar_time > 100){
      shift_integrator += shift * 0.008895;
      shift_integrator_intervar_time = millis();
    }
    ...
    moveAngle(a + shift + shift_integrator, forward_v);
  } else {
