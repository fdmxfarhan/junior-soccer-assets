...
void turn_catch() {
  ...
  gyro_reverse = false;
  if (ball_distance < 60 && abs(ball_angle) < 20) {
    moveSec(ball_angle, HIGH_SPEED, 14);
  } else {
    float Vx = (robot_x - ball_x) * 5000;
    float Vy = (robot_y - ball_y) * 5000;
    float w = ball_angle * 3000;
    moveXY(Vx, Vy, w);
  }
}
void goal() {
...
void defend_goal() {
  ...
  if (BALL_IN_KICKER) {
    ...
    if (ball_distance < 65) {
      ...
      if (shl < 500 && Vx > 0) Vx = 0;
      if (shr < 500 && Vx < 0) Vx = 0;
