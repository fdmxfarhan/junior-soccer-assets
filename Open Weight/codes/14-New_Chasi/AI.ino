...
void goal() {
  if(go_forward_catch_ball < 30){
    move_angle_speed(0, LOW_SPEED);
    go_forward_catch_ball++;
    return;
  }
  // if (already_shooted) {
  //   //////////////////////////////////////////// Backward Move
  ...
  // } else {
    ...
    if (is_goal && SHOOT_TO_GOAL) {
      ...
      if (goal_angle <= 4 && goal_angle >= -4) {
        for (int i = 0; i < 5; i++) {
          update_all();
          moveXY(0, 0, clamp(goal_angle * 3, -LOW_SPEED, LOW_SPEED));
          SPIN_OFF;
          iwdg_feed();  // reload watchdog
        }
        stop();
        shoot();
        already_shooted = true;
      } else moveXY(0, 0, clamp(goal_angle * 3, -LOW_SPEED, LOW_SPEED));
    }
    //////////////////////////////////////////// No Goal
    else {
      stop();
      ...
      // goal_detected = false;
      // update_all();
      // if (abs(robot_angle) < 10 && abs(d) < 200) {
      //   shoot();
      //   already_shooted = true;
      // } else if (abs(robot_angle) > 60) move_angle_speed(robot_angle, MID_SPEED);
      // else moveXY(d, MID_SPEED, 0);
    }
  // }
}
void goal_old() {
...
void Rotate_Move_AI() {
  ...
  if (BALL_IN_KICKER) {
    ...
    go_forward_catch_ball = 0;
    if (ball_distance < OUT_OF_SIGHT && abs(ball_angle) <= 40) {
      for (int i = 0; i < 20; i++) {
        iwdg_feed();  // reload watchdog
