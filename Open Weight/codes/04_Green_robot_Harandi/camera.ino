  ...
  if (blocks) {
    for (int j = 0; j < blocks; j++) {
      if (pixy.blocks[j].signature == 1) {
        is_ball = true;
        ball_x = pixy.blocks[j].x;
        ball_y = pixy.blocks[j].y;
        ball_angle = degrees(atan2(robot_y - ball_y, ball_x - robot_x));
        ball_distance = sqrt(pow(robot_x - ball_x, 2) + pow(robot_y - ball_y, 2));

      } else if (pixy.blocks[j].signature == 2) {
        is_yellow = true;
        yellow_x = pixy.blocks[j].x;
        yellow_y = pixy.blocks[j].y;
        yellow_angle = degrees(atan2(yellow_x - robot_x, robot_y - yellow_y));
      } else if (pixy.blocks[j].signature == 3) {
        is_blue = true;
        blue_x = pixy.blocks[j].x;
        blue_y = pixy.blocks[j].y;
        blue_angle = degrees(atan2(blue_x - robot_x, robot_y - blue_y));
      }
    }
    if (Team_Yellow) {
      is_goal = is_yellow;
