  ...
  if (already_shooted) {
    if(Back_Spin){
      ...
      if (arrived_to_goal) {
        ...
        TDAxis_reverse = false;
        TDAxis_enable = true;
        if( goal_time_out > 80) already_shooted = false;
        else move_angle_speed(0, MID_SPEED);
        goal_time_out++;
    }
  } else {
    if (is_goal) {
      ...
      } else moveXY(0, 0, clamp(goal_angle*2, -MID_SPEED, MID_SPEED));
    }
    //////////////////////////////////////////// No Goal
    else {
      TDAxis_enable = true;
      goal_detected = false;
      // update_all();
      if (abs(robot_angle) < 5 && abs(d) < 300) {
        shoot();
        already_shooted = true;
      } else if (abs(robot_angle) > 50) stop();  //move_angle_speed(robot_angle, LOW_SPEED);
