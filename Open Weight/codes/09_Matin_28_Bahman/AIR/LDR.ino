...
void out() {
  ...
  else if (ldr.front > LDR_SENS && shr < shl) {moveSec(225, HIGH_SPEED, MOVE_INSIDE_SEC); out_ball_angle = 0;}
  else if (ldr.front > LDR_SENS && shr > shl) {moveSec(135, HIGH_SPEED, MOVE_INSIDE_SEC); out_ball_angle = 0;}
  else if (ldr.right > LDR_SENS) {moveSec(270, HIGH_SPEED, MOVE_INSIDE_SEC); out_ball_angle = 90;}
  else if (ldr.back >= LDR_SENS && shr > shl) {moveSec(45, HIGH_SPEED, MOVE_INSIDE_SEC*1.5); out_ball_angle = 180;}
  else if (ldr.back >= LDR_SENS && shl < shr) {moveSec(315, HIGH_SPEED, MOVE_INSIDE_SEC*1.5); out_ball_angle = 180;}
